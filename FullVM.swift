...
struct FullVM {
    ...
    mutating func executeBinary() {
        ...
        while(pointerIsInMemoryBounds()) {
            ...
            return //halt
            case Instructions.clrr.rawValue: clrr(memory[instructionPointer + 1])
            case Instructions.clrx.rawValue: clrx(memory[instructionPointer + 1])
            case Instructions.clrm.rawValue: clrm(memory[instructionPointer + 1])
            case Instructions.clrb.rawValue: clrb(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movir.rawValue: movir(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movrr.rawValue: movrr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movrm.rawValue: movrm(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movmr.rawValue: movmr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movxr.rawValue: movxr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movar.rawValue: movar(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movb.rawValue: movb(memory[instructionPointer + 1], memory[instructionPointer + 2], memory[instructionPointer + 3])
            case Instructions.addir.rawValue: addir(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.addrr.rawValue: addrr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.addmr.rawValue: addmr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.addxr.rawValue: addxr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.subir.rawValue: subir(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.subrr.rawValue: subrr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.submr.rawValue: submr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.subxr.rawValue: subxr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.mulir.rawValue: mulir(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.mulrr.rawValue: mulrr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.mulmr.rawValue: mulmr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.mulxr.rawValue: mulxr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.divir.rawValue: divir(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.divrr.rawValue: divrr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.divmr.rawValue: divmr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.divxr.rawValue: divxr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.jmp.rawValue: jmp(memory[instructionPointer + 1])
            case Instructions.sojz.rawValue: sojz(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.sojnz.rawValue: sojnz(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.aojz.rawValue: aojz(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.aojnz.rawValue: aojnz(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.cmpir.rawValue: cmpir(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.cmprr.rawValue: cmprr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.cmpmr.rawValue: cmpmr(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.jmpn.rawValue: jmpn(memory[instructionPointer + 1])
            case Instructions.jmpz.rawValue: jmpz(memory[instructionPointer + 1])
            case Instructions.jmpp.rawValue: jmpp(memory[instructionPointer + 1])
            case Instructions.jsr.rawValue: jsr(memory[instructionPointer + 1])
            case Instructions.ret.rawValue: ret()
            case Instructions.push.rawValue: push(memory[instructionPointer + 1])
            case Instructions.pop.rawValue: pop(memory[instructionPointer + 1])
            case Instructions.stackc.rawValue: stackc(memory[instructionPointer + 1])
            case Instructions.outci.rawValue: outci(memory[instructionPointer + 1])
            case Instructions.outcr.rawValue: outcr(memory[instructionPointer + 1])
            case Instructions.outcx.rawValue: outcx(memory[instructionPointer + 1])
            case Instructions.outcb.rawValue: outcb(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.readi.rawValue: readi(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.printi.rawValue: printi(memory[instructionPointer + 1])
            case Instructions.readc.rawValue: readc(memory[instructionPointer + 1])
            case Instructions.readln.rawValue: readln(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.brk.rawValue: brk()
            case Instructions.movrx.rawValue: movrx(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.movxx.rawValue: movxx(memory[instructionPointer + 1], memory[instructionPointer + 2])
            case Instructions.outs.rawValue: outs(memory[instructionPointer + 1])
            case Instructions.nop.rawValue: nop()
            case Instructions.jmpne.rawValue: jmpne(memory[instructionPointer + 1])
            default:
